...
The Docker socket is a Critical asset. The backend can list, inspect, start, stop and restart containers today, uses Docker exec for fixed Palworld update checks, and GH-15 can acquire a durable backend-approved public image digest. The implementation exposes no arbitrary Docker create, exec, mount, image pull or volume API to clients.
...
Remaining risks include image signing and registry trust, Docker-root capacity planning, symlink/junction and TOCTOU filesystem hardening, configurable host-wide resource quotas, authorization and the inherent privilege of the GamesHud backend Docker socket. ARCH-03 adds durable digest/platform identity and GH-15 adds public digest-only acquisition with final inspect; Palworld activation remains gated on an approved digest.
...
| THR-027 | Partial provisioning failure | Provisioning | Directory/container/image acquired but API reports success or retries unsafely. | Orphaned resources, duplicate/conflicting identity, data loss. | Medium | High | GH-09 checkpoints unknown effects; GH-11/GH-12 provide trusted inspection; ARCH-03 applies and audits evidence; GH-15 re-inspects the durable digest, restores verified local identity after crashes and permits retry only after proven absence. | No automatic orchestration or safe owned-resource deletion/rollback. | Explicit recovery orchestration and owned-resource rollback. | Future recovery work |
...
| THR-031 | Runtime environment injection or drift | Runtime creation | A client/caller adds environment, influences image behavior, leaks a secret through inspect, or omits an image-required setting so generated configuration overwrites managed state. | Host/runtime policy bypass, secret disclosure, or managed configuration loss. | Low | High | Environment is non-secret definition metadata, absent from HTTP requests, syntax/size bounded, exact-matched by SEC-03, immutable after validation, and compared during reconciliation without logging values. Palworld fixes `DISABLE_GENERATE_SETTINGS=true`; ARCH-03 V2 pins image digest/platform and verifies the local image id. | Docker inspect remains privileged, host/process administrators can observe non-secret environment, V1 retains its historical tag contract, and other catalog metadata is not yet versioned with image intent. | Keep definitions reviewed, prohibit secret routing, activate V2 only with an approved digest, and add provenance/signature controls. | ARCH-02/ARCH-03 |
| THR-032 | Durable game configuration confusion or disclosure | Provisioning persistence | Raw/malformed JSON, wrong game/schema, cross-server reuse, confused secret roles, or payload logging changes expected configuration or exposes identifiers/plaintext. | Incorrect server configuration or credential exposure. | Low | High | Trusted typed codec, unknown-property rejection, explicit secret roles, opaque IDs only, ownership checks, version checks, unique kind constraint, transactional creation, and safe errors without payloads. | Database readers can observe non-secret settings and opaque secret IDs; process administrators remain trusted. | Keep payload access backend-only and add authorization/audit before public configuration APIs. | GH-14A |
