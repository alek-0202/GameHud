...
Resource checks use game minimum requirements when available and map only CPU count and memory. Before create, managed mount directories must exist and their ancestry must contain no reparse point. Docker socket sources and targets are rejected. ARCH-03 provides typed digest/platform pinning and durable image intent, but performs no image acquisition. Signature verification, registry authentication and stronger provider-specific quota controls remain future work.
