...
Resource checks use game minimum requirements when available and map only CPU count and memory. Before create, managed mount directories must exist and their ancestry must contain no reparse point. Docker socket sources and targets are rejected. ARCH-03 provides typed digest/platform pinning and durable image intent; GH-15 acquires only that public digest and proves it by final inspect. Signature verification, registry authentication and stronger provider-specific quota controls remain future work.
...
For `gh15-v2`, runtime specifications are reconstructed with the immutable approved image intent and a separately verified local Docker image id from durable persistence. Container creation addresses that local id directly, and inspect must return the same id before create reconciliation or start can accept the container. The current production `gh09-v1` path continues to use its legacy trusted reference. See [Durable Runtime Image Identity](../runtime-image-identity.md).

The GH-15 adapter exposes only typed inspect and acquire operations. Its infrastructure implementation calls Docker image inspect and create/pull with the exact durable digest and platform and a null authentication configuration. It does not expose or implement tag, build, import, load, push, remove or prune. Raw progress and provider errors do not cross the boundary.
